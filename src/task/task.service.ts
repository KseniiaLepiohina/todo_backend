...
export class TaskService {
  constructor(private readonly dataSource: DataSource,
    @InjectRepository(ActiveTasks) private ActiveTaskRepository:Repository<ActiveTasks>,
    @InjectRepository(CompletedTasks) private CompletedTasksRepository:Repository<CompletedTasks>
  ) { }
  ...
  async addToCompleteTasks(task: CreateCompletedTaskDto) {
    try {
      const insertIntoCompletedTasks = await this.dataSource
        .createQueryBuilder()
        .insert()
        .into(CompletedTasks)
        .values([{
        ...
        .returning('*')
        .execute();
      return insertIntoCompletedTasks.raw[0];
    } catch (error) {
      throw new HttpException('Failed to add into completed tasks', HttpStatus.BAD_REQUEST);
    }
  }
